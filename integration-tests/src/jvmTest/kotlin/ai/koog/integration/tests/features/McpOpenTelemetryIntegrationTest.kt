...
class McpOpenTelemetryIntegrationTest {
    ...
    private fun createAgentWithMcpAndOtel(
        ...
        return AIAgent(
            ...
            toolRegistry = ToolRegistry { tool(CalculatorTool) }
        ) {
            ...
            install(Mcp) {
                addMcpServerFromTransport(
                    transport = sseClientTransport(McpServerPort),
                    serverInfo = McpServerInfo(McpServerName, "http://localhost:$McpServerPort", McpServerPort)
                )
            }
        }
    }
