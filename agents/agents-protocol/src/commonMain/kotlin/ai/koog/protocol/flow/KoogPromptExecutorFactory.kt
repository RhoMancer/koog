...
public object KoogPromptExecutorFactory {
     ...
     */
    private object DEFINED_API_KEYS {
    ...
    public fun resolveModel(modelString: String?, defaultModel: String?): LLModel {
        ...
        val model = getModelFromIdentifier(fullModelIdentifier, delimiter = "/")
            ?: getModelFromIdentifier(fullModelIdentifier)
            ?: error("Invalid model string: $fullModelIdentifier")
     ...
     *
     * This method:
     * 1. Extracts unique providers from the models
     * 2. Creates one LLMClient per provider by reading credentials from environment variables
     * 3. Returns a MultiLLMPromptExecutor configured with all created clients
     *
    ...
    private fun createOpenAIClient(): LLMClient? {
        val apiKey = getEnvironmentVariableOrNull(DEFINED_API_KEYS.OPENAI_API_KEY) ?: run {
            logger.error { "Missing environment variable: ${DEFINED_API_KEYS.OPENAI_API_KEY}" }
            return null
        }
    ...
    private fun createAnthropicClient(): LLMClient? {
        val apiKey = getEnvironmentVariableOrNull(DEFINED_API_KEYS.ANTHROPIC_API_KEY) ?: run {
            logger.error { "Missing environment variable: ${DEFINED_API_KEYS.ANTHROPIC_API_KEY}" }
            return null
        }
    ...
    private fun createGoogleClient(): LLMClient? {
        val apiKey = getEnvironmentVariableOrNull(DEFINED_API_KEYS.GOOGLE_API_KEY) ?: run {
            logger.error { "Missing environment variable: ${DEFINED_API_KEYS.GOOGLE_API_KEY}" }
            return null
        }
    ...
    private fun createMistralClient(): LLMClient? {
        val apiKey = getEnvironmentVariableOrNull(DEFINED_API_KEYS.MISTRAL_API_KEY) ?: run {
            logger.error { "Missing environment variable: ${DEFINED_API_KEYS.MISTRAL_API_KEY}" }
            return null
        }
    ...
    private fun createDeepSeekClient(): LLMClient? {
        val apiKey = getEnvironmentVariableOrNull(DEFINED_API_KEYS.DEEPSEEK_API_KEY) ?: run {
            logger.error { "Missing environment variable: ${DEFINED_API_KEYS.DEEPSEEK_API_KEY}" }
            return null
        }
    ...
    private fun createOpenRouterClient(): LLMClient? {
        val apiKey = getEnvironmentVariableOrNull(DEFINED_API_KEYS.OPENROUTER_API_KEY) ?: run {
            logger.error { "Missing environment variable: ${DEFINED_API_KEYS.OPENROUTER_API_KEY}" }
            return null
        }
    ...
    private fun createOllamaClient(): LLMClient {
        ...
        val baseUrl = getEnvironmentVariableOrNull(DEFINED_API_KEYS.OLLAMA_BASE_URL) ?: "http://localhost:11434"
