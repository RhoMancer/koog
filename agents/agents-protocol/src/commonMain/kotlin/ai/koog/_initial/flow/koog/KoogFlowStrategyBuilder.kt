...
/**
 * Builder that converts flow configuration (agents + transitions) into a single
 * [AIAgentGraphStrategy] for execution by a Koog AIAgent.
 */
public class KoogFlowStrategyBuilder {
    ...
    /**
     * Builds an [AIAgentGraphStrategy] from the flow configuration.
     *
     ...
     * @param tools List of tools available to the flow
     * @param defaultModel Default model string for agents that don't specify one
     * @return A strategy that can be executed by an AIAgent
     */
    public fun build(
        id: String,
        ...
        // No agents
        if (agents.isEmpty()) {
            logger.warn { "No agents provided, creating empty strategy for flow '$id'" }
            return createEmptyStrategy(id)
        }

        // No transitions
        if (transitions.isEmpty()) {
            logger.warn { "No transitions provided, creating empty strategy for flow '$id'" }
            return createEmptyStrategy(id)
        }
