    ...
    fun `test filter is not allowed for debugger feature`() = runBlocking {
        ...
        val serverJob = launch {
            ...
            createAgent(
                ...
                agent.run("Test user input")
                isClientFinished.await()
            }
        }

        var collectedClientEvents = listOf<FeatureMessage>()
        ...
        val clientJob = launch {
            FeatureMessageRemoteClient(
                ...
                val clientEventsCollector = ClientEventsCollector(client = client, expectedEventsCount = 1)
                val collectEventsJob = clientEventsCollector.startCollectEvents(coroutineScope = this@launch)
                collectedClientEvents = clientEventsCollector.collectedEvents
        ...
        assertNotNull(isFinishedOrNull, "Client or server did not finish in time")
        assertTrue(collectedClientEvents.isNotEmpty(), "Expected non empty list of events, but got none")
    }
